  ...
  const fetchFocusData = async () => {
    ...
    try {
      ...
      const inProgressResponse = await fetch(`${BASE_URL}/getInProgressItems`, {
        ...
        body: JSON.stringify({
          date: formattedDate,
          in_progress_items: {}, // Changed from [] to {}
        }),
      });
      console.log("getInProgressItems response status:", inProgressResponse.status);
      if (!inProgressResponse.ok) {
      ...
      const processTasks = (data: any): Task[] => {
        if (data.status === 200 && data.reports && data.reports.length > 0) {
          // Assuming data.reports[0] is now a dictionary like { "item_id_1": {...}, "item_id_2": {...}, ... }
          const itemsDict = data.reports[0];
          if (typeof itemsDict === "object" && itemsDict !== null) {
            return Object.values(itemsDict).map((item: any) => ({
              id: String(item.id),
              name: item.context.name,
              isFocused: item.context.is_focused || false,
              parentId: item.context.parent_item_id || null,
          ...
          <div style={{ flex: 1 }}>
            <div style={{ backgroundColor: "#f9f9f9", borderRadius: "8px", marginBottom: "20px", padding: "15px" }}>
              ...
              {todaysFocus.length > 0 ? (
                ...
                <p style={noTasksStyle}>No focused tasks for today.</p>
              )}
            </div>
            ...
            <div style={{ backgroundColor: "#f9f9f9", borderRadius: "8px", marginBottom: "20px", padding: "15px" }}>
              ...
              {inProgress.length > 0 ? (
                ...
                <p style={noTasksStyle}>No in-progress tasks.</p>
              )}
            </div>
          </div>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ backgroundColor: "#f9f9f9", borderRadius: "8px", marginBottom: "20px", padding: "15px" }}>
              ...
              {todaysRituals.length > 0 ? (
                ...
                <p style={noTasksStyle}>No ritual tasks for today.</p>
              )}
            </div>
            ...
            <div style={{ backgroundColor: "#f9f9f9", borderRadius: "8px", marginBottom: "20px", padding: "15px" }}>
              ...
              {todaysRecurrings.length > 0 ? (
                ...
                <p style={noTasksStyle}>No recurring tasks for today.</p>
              )}
            </div>
          </div>
        </div>
