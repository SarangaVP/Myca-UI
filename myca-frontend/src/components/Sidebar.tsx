...
export const Sidebar: React.FC = () => {
  ...
  const navigateTo = (item: string, path: string) => {
    ...
    navigate(path);
    if (isMobile) {
      toggleSidebar(); // Auto-collapse sidebar on mobile after navigation
    }
  };

  return (
    <div style={sidebarStyle(isCollapsed)} className={`sidebar ${isCollapsed ? "collapsed" : ""}`}>
      {/* Toggle Button (now inside the sidebar, on the left) */}
      <button onClick={toggleSidebar} style={toggleButtonStyle(isCollapsed)} className="toggle-button">
        {isCollapsed ? <FaBars /> : <FaTimes />}
      </button>
      ...
      {!isCollapsed && (
        <>
          <h1 style={titleStyle} className="navbar-brand">
            Myca
          </h1>
          <ul style={{ listStyle: "none", padding: "0", flexGrow: 1 }}>
            <li style={sectionTitleStyle} className="section-title">
              Perform
            </li>
            <li
              style={menuItemStyle(activeItem === "Plan")}
...
const sidebarStyle = (isCollapsed: boolean): CSSProperties => ({
  width: isCollapsed ? "60px" : "250px", // Keep a small width when collapsed to show the toggle button
...
const toggleButtonStyle = (isCollapsed: boolean): CSSProperties => ({
  backgroundColor: "transparent",
  color: "#333",
  border: "none",
  fontSize: "20px",
