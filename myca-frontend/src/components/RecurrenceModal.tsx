...
const RecurrenceModal: React.FC<RecurrenceModalProps> = React.memo(({ isOpen, onClose, task, refreshTasks }) => {
  const [repeatEvery, setRepeatEvery] = useState(1);
  const [repeatUnit, setRepeatUnit] = useState("DAYS");
  const [startDate, setStartDate] = useState(new Date().toISOString().split("T")[0]);
  const [endDate, setEndDate] = useState("");
  ...
  // Fetch recurrence data manually when needed
    ...
    try {
      ...
      if (data.status === 200 && data.reports.length > 0) {
        ...
        if (ritual) {
          ...
          setByDayOfMonth(ritual.by_day_of_month || 1);
          setIsRitual(ritual.ritual_flag || false);
          setByDayOfWeek(ritual.by_day_of_week || [false, false, false, false, false, false, false]);
          if (ritual.end) {
            setEndType("ON");
            setEndDate(ritual.end);
          } else if (ritual.occurrence) {
  ...
  useEffect(() => {
    console.log("RecurrenceModal rendered, isOpen:", isOpen);
    if (isOpen && !isDataLoaded) {
      fetchRecurrence();
    }
  }, [isOpen, isDataLoaded, fetchRecurrence]);
  ...
  }, [task.id, startDate, repeatUnit, isRitual, byDayOfWeek, byDayOfMonth, repeatEvery, endDate, occurrences, endType, refreshTasks, onClose]);

  const toggleDayOfWeek = useCallback((index: number) => {
    setByDayOfWeek((prevDays) =>
      prevDays.map((selected, i) => (i === index ? !selected : selected))
    );
  }, []);
